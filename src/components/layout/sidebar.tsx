...
export function AdminSidebar({ className }: { className?: string }) {
  ...
  return (
    // FIXED: Default to bg-white, but allow className to override (for transparent mobile use)
    <aside className={cn("flex flex-col h-full bg-white dark:bg-slate-950 border-r border-slate-200", className)}>
      ...
      <div className="p-4 border-t border-slate-200 shrink-0">
        <button className="flex w-full items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
